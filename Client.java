...
public class Client {
    ...
    private void listenForServerMessages() {
        try {
            ...
            while ((response = in.readLine()) != null) {
                ...
                } else if (response.startsWith("UPDATED_MESSAGE:")) {
                    String updatedMessage = response.substring("UPDATED_MESSAGE:".length());
                    for (int i = 0; i < serverMessageCount; i++) {
                        if (serverMessages[i] != null && updatedMessage.startsWith(serverMessages[i].split("\\|")[0].trim())) {
                            serverMessages[i] = updatedMessage;
                            break;}
                    }
                    for (int i = 0; i < clientMessageCount; i++) {
                        if (clientMessages[i] != null && updatedMessage.startsWith(clientMessages[i].split("\\|")[0].trim())) {
                            clientMessages[i] = updatedMessage;
                            break;
                        }
                    }
                } else {
    ...
    private void addReaction() {
        ...
        // Loop through the messages and check for matching ID
        for (int i = 0; i < messageCount; i++) {
    ...
    private String extractMessageID(String message) {
        // Split the message string by the delimiter " | "
    ...
    private void editMessage() {
        ...
        out.println("EDIT:" + messageId + ":" + newContent);
